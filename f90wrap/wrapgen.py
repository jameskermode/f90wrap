    ...
    def visit_Subroutine(self, node):
        ...
        self.write("subroutine %(sub_name)s(%(arg_names)s)" %
                   {'sub_name': self.prefix+node.name,
                    'arg_names': ', '.join([self.prefix+arg.name for arg in node.arguments])})
        self.indent()
        self.write()
...
def transform_to_f90_wrapper(tree, types, kinds, callbacks, initlines):
    """
    ...
    tree = UnwrappablesRemover(callbacks, types).visit(tree)
    tree = FunctionToSubroutineConverter().visit(tree)
    tree = fix_subroutine_uses_clauses(tree, types, kinds)
    tree = convert_derived_type_arguments(tree, initlines)
    return tree
