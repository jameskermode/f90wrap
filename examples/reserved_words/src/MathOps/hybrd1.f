      subroutine hybrd1(fcn,n,x,fvec,tol,info,nfev,maxfev,epsfcn)
      integer n,info,nfev,maxfev
      real tol
      real x(n),fvec(n)

! modified wrapper routine for hybrd -> removes extra work array args 
! from interface
      integer :: lwa
      real    :: wa((n*(3*n+13))/2),epsfcn
      external fcn
...
c
c     **********
      integer index,j,lr,ml,mode,mu,nprint
...
c     call hybrd.
c
! AS catch for setting default value of maxfev
      if (maxfev .le. 0) maxfev = 200*(n + 1)
      
      xtol = tol
      ml = n - 1
      mu = n - 1
C       epsfcn = 5.e-3       ! AS: set eps to 0.05
      mode = 2
      do 10 j = 1, n
         wa(j) = one
   10    continue
